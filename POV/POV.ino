...
void setup() {
  ...
  //GPS Daten empfangen
  while (!gps.time.isUpdated() && millis() < 5000) {
...
void strToAry(char *s, int start, int r, int g, int b) {
  while ( *s != '\0') {
    ...
    *s++;
    //eine Spalte Abstand zwischen den Buchstaben
    //die spalte extra als leer einfügen, damit beim überschreiben keine artefakte auftreten
    //byteToRGB(0,start+5,r,g,b);
    start += 6;
  }
